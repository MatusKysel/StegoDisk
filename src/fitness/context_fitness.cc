...
ContextFitness::ContextFitness(std::shared_ptr<CarrierFile> file)
    : Fitness(file) {}

ContextFitness::~ContextFitness() {}

bool ContextFitness::CheckSubboxValidity(
    const std::array<unsigned char, 4> &subbox) const {
...
bool ContextFitness::CheckValidity(
    const std::array<unsigned char, 9> &box) const {
  bool zero, one;
  unsigned char central_pixel;

  central_pixel = box[4] & 0xFE;
  zero = (CheckSubboxValidity(std::array<unsigned char, 4>{
              {box[0], box[1], box[3], central_pixel}}) &&
          CheckSubboxValidity(std::array<unsigned char, 4>{
              {box[1], box[2], central_pixel, box[5]}}) &&
          CheckSubboxValidity(std::array<unsigned char, 4>{
              {box[3], central_pixel, box[6], box[7]}}) &&
          CheckSubboxValidity(std::array<unsigned char, 4>{
              {central_pixel, box[5], box[7], box[8]}}));

  central_pixel |= 0x01;

  one = (CheckSubboxValidity(std::array<unsigned char, 4>{
             {box[0], box[1], box[3], central_pixel}}) &&
         CheckSubboxValidity(std::array<unsigned char, 4>{
             {box[1], box[2], central_pixel, box[5]}}) &&
         CheckSubboxValidity(std::array<unsigned char, 4>{
  ...
  if (file_->IsGrayscale()) {
    ...
    for (uint64 i = 0; i < i_max; ++i) {
      ...
      for (uint64 j = 0; j < j_max; ++j) {
        ...
        if (CheckValidity(std::array<unsigned char, 9>{
                {in[index - file_->GetWidth() - 1],
                 in[index - file_->GetWidth()],
                 in[index - file_->GetWidth() + 1], in[index - 1], in[index],
                 in[index + 1], in[index + file_->GetWidth() - 1],
                 in[index + file_->GetWidth()],
                 in[index + file_->GetWidth() + 1]}})) {
          ++counter;
...
void ContextFitness::InsertBytes(const MemoryBuffer &in,
                                 MemoryBuffer *out) const {
  if (file_->IsGrayscale()) {
