...
KeccakHashImpl::KeccakHashImpl(std::size_t state_size) {
  ...
  if (state_size == 0 || state_size >= 100 || (state_size % 4) != 0)
    throw std::invalid_argument("KeccakHashImpl: unsupported state size " +
...
void KeccakHashImpl::Process(MemoryBuffer& state, const uint8* data,
                             std::size_t length) {
  ...
  if (keccak(data, static_cast<int>(length), state.GetRawPointer(),
             static_cast<int>(state_size_)) != 0)
