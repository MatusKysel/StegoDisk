...
#define DEFINE_GETTER(member_name)                                             \
  auto get_##member_name() const {                                             \
    return member_name##_;                                                     \
  }
#define DEFINE_STRING_GETTER(member_name)                                      \
  const char* get_##member_name() const {                                      \
    return member_name##_.c_str();                                             \
  }
...
public:
  ...
  ErrorOpenFIle(const std::string& path, const std::string& reason)
      : std::runtime_error("File '" + path +
                           "' can not be opened bacouse of '" + reason + "'"),
        path_{path}, reason_{reason} {}
...
public:
  HashSizeTooSmall() : std::runtime_error("Hash size is too small") {}
...
public:
  IoError(const std::string& file)
      : std::runtime_error("IO for file '" + file + "' failed"), file_{file} {}
