...
public:
  ...
  inline static void Init(const json::JsonObject &config) {
    ...
    Instance().encoder_ =
        EncoderFactory::GetEncoderType(config["encoder"].ToString());
    Instance().global_perm_ =
        PermutationFactory::GetPermutationType(config["glob_perm"].ToString());
    Instance().local_perm_ =
        PermutationFactory::GetPermutationType(config["local_perm"].ToString());
        ...
        if (file_types[i].IsObject()) {
          if (file_types[i]["file_type"].IsString()) {
            Instance().file_config_[file_types[i]["file_type"].ToString()] =
                std::make_pair(EncoderFactory::GetEncoderType(
                                   file_types[i]["encoder"].ToString()),
                               PermutationFactory::GetPermutationType(
                                   file_types[i]["permutation"].ToString()));
          }
        }
      }
    }
  ...
  inline static PermutationFactory::PermutationType &global_perm() {
    return Instance().global_perm_;
  }
  inline static PermutationFactory::PermutationType &local_perm() {
    return Instance().local_perm_;
  }
  inline static EncoderFactory::EncoderType &encoder() {
    return Instance().encoder_;
  }
  inline static std::set<std::string> &exclude_list() {
    return Instance().exclude_list_;
  }
  inline static std::map<std::string,
                         std::pair<EncoderFactory::EncoderType,
                                   PermutationFactory::PermutationType>> &
  file_config() {
    return Instance().file_config_;
  }


private:
  StegoConfig()
      : stego_config_loaded_(false), exclude_list_(), file_config_() {}
  ...
  std::map<std::string, std::pair<EncoderFactory::EncoderType,
                                  PermutationFactory::PermutationType>>
      file_config_;
